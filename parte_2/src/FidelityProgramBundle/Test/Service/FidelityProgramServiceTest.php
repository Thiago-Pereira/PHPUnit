...
class FidelityProgramServiceTest extends TestCase
{
    ...
    public function shouldSaveWhenReceivePoints()
    {
        ...
        $pointsRepository->expects($this->once()) // espero uma vez que o metódo save seja chamado
            ->method('save');
        ...
        $logger->method('log')
            ->will($this->returnCallback(
                function ($message) use (&$allMessages) { // um Spy, para sabermos todas as mensagens que são passadas no log
                    $allMessages[] = $message;
                }
            ));
        ...
        $customer = $this->createMock(Customer::class); // um Dummie
        $value = 50;
    ...
    {
        $pointsRepository = $this->createMock(PointsRepository::class);
        $pointsRepository->expects($this->never()) // espero que nunca seja chamado
            ->method('save');
